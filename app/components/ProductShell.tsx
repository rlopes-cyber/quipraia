...
export function ProductShell({ active, title, eyebrow, children }: { active: string; title: string; eyebrow: string; children: ReactNode }) {
  ...
  return <main className="product-shell">
    <aside className="product-sidebar"><a href="/app"><img src="/brand/final/quipraia-3c-wordmark-dark-approved.svg" alt="QuiPraia" /></a><small>Previsão de surf</small><nav>{items.map(([label, href, icon]) => <a className={active === label ? "active" : ""} href={href} key={label}><Icon name={icon} />{label}</a>)}</nav><div className="product-user"><b>RL</b><span><strong>Ricardo</strong><small>Colaborador</small></span></div></aside>
