...
export function CompareExperience() {
  ...
  const rows = selected.map((beach, index) => {
    ...
    const period = point?.wavePeriod ?? beach.period;
    ...
    return { beach, wave, period, wind, tide, tideRising, score, isLive: forecast.isLive };
  });
  ...
  return <section className="compare-experience">
    ...
    <div className="compare-grid">{rows.map((row, index) => { const { beach, wave, period, wind, tide, tideRising, score } = row; const isBest = index === bestIndex; return <article className={isBest ? "best" : ""} key={beach.slug}><img className="compare-photo" src={beach.image} style={{ objectPosition: beach.imagePosition }} alt={`Vista editorial de ${beach.name}`} /><header><div><span className="hot-kicker">{isBest ? "Melhor escolha" : "Comparação"}</span><h2>{beach.name}</h2></div><b>{score}<small>/100</small></b></header><div className="compare-wave"><DataIcon name="waves" /><strong>{wave.toFixed(1)} m</strong><span>{Math.round(period)} s · {beach.windDirection}</span></div><dl><div><dt>Vento</dt><dd>{beach.windDirection} · {wind} km/h</dd></div><div><dt>Maré</dt><dd>{tide.toFixed(1)} m {tideRising ? "subindo" : "descendo"}</dd></div><div><dt>Fundo</dt><dd>{beach.surf?.bottomType ?? "Sem fonte confiável"}</dd></div><div><dt>Melhor para</dt><dd>{beach.surf?.skillLevelShort ?? "Sem fonte confiável"}</dd></div></dl><div className="score-track"><i style={{ width: `${score}%` }} /></div><a href={`/praias/${beach.slug}`}>Ver praia</a></article>; })}</div>
