...
export function BeachDetails({ beach }: { beach: Beach }) {
  ...
  return <>
    <section className="beach-hero"><img src="/images/quipraia-stella-maris-hero-v1.png" alt={`Ondas em ${beach.name}`} /><div><span className="hot-kicker">Praia em destaque</span><h2>{beach.name}</h2><p>Salvador · atualização há 8 min</p><b className="condition-pill bom">● {beach.condition}</b><div className="beach-pulse"><small>Session Pulse</small><i><span style={{ width: `${beach.score}%` }} /></i><strong>{beach.score}/100</strong></div></div></section>
