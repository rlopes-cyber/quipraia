...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json({ beach: { name: beach.name, slug: beach.slug }, model: "Open-Meteo (onda/vento/temp.) + cálculo harmônico de maré (TICON-4, estação de Salvador)", notice: "Previsão modelada. Não usar para navegação marítima.", updatedAt: new Date().toISOString(), points });
  } catch (error) {
